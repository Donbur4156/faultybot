...
from interactions import (Intents, SlashContext, Task, TimeTrigger, listen,
...
@teamname_option()
@slash_option(name="oauth", description="OAuth Code", opt_type=di.OptionType.STRING, required=True)
async def kickfaulty(ctx: SlashContext, teamname: str, oauth: str):
    teamname = function.check_team_name(teamname.lower())
    text = f"The data of the team **{teamname}** is downloaded and checked!\nThis can take several " \
...
@teamname_option()
@slash_option(name="user", description="user who will be pinged", opt_type=di.OptionType.USER, required=True)
...
@cron_cmds.subcommand(sub_cmd_name="get_cron_teams", sub_cmd_description="get all teams which are checked daily")
...
async def run_kick(ctx: SlashContext, team: str, cheaters: list, token: str):
    ...
    for cheater in cheaters:
        ...
        bot_logger.info(f"Request for Cheater {count_cheater + 1}. {cheater}: returns {request.text}")
...
async def cron_faulty():
    ...
    for team in json_data['teams']:
        ...
        if cheaters:
            data = "\n".join([f"[{cheater}](https://lichess.org/@/{cheater})" for cheater in cheaters])
            bot_logger.info(f'{str(len(cheaters))} cheater found in team "{teamname}": {cheaters}')
            user_mentions = ", ".join([f'<@{user}>' for user in userlist])
            text = f"In the team **{teamname}**, " \
                f"following users were marked by Lichess as having violated the terms of use:\n{data}\n"
            await request_channel.send(content=user_mentions, embed=_embed(text, Colors.ORANGE))
