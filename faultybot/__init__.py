...
async def kickfaulty(ctx, *args):
    ...
    if channel:
        text = (
            "This command is only available via private message.. "
    ...
    if len(args) != 2:
        text = (
            "The command ``>kickfaulty`` requires 2 arguments: "
    ...
    text = (
        f"The data of the team **{team}** is downloaded and checked! This can take several "
...
async def faulty(ctx, *args):
    if len(args) < 1 or len(args) > 2:
        text = (
            "The command ``>faulty`` requires at least 1 argument: "
    ...
    if id_ref[handle][3] == 1:  # team neu
        text = (
            f"The data of the team **{team}** is downloaded and checked! "
...
async def add(ctx, teamname, userid):
    ...
    for team in json_data["teams"]:
        if team["teamname"] == teamname:
            ...
            break
    if teamnew:
        json_data["teams"].append({"teamname": teamname, "user": [userid]})
    with open(TEAM_FILE, "w", encoding="utf-8") as json_file:
        json.dump(json_data, json_file)
    mention = "<@" + str(userid) + "> "
    text = (
        f"das Team {teamname} ist nun mit dem User mit dem User {mention} "
...
async def add_error(ctx, error):
    if isinstance(error, discord.ext.commands.MissingRole):
        await ctx.send(
            "Du hast nicht die benötigten Rechte um diesen Command auszuführen!"
        )
...
async def faultyhandle(ctx, team, handle):
    ...
    try:
        ...
        cheaters = await loop.run_in_executor(
            ThreadPoolExecutor(), function.analyse_team, team, ignore_user
        )
...
async def run_kick(ctx, team, handle, cheaters, token):
    ...
    if count_cheater == 1:
        text = (
            f"There was 1 of {str(len(cheaters))} flagged user kicked\n"
            " - - - - - - - - - - - - - - - - - - - - - - - - - - - - "
        )
    else:
        text = (
            f"There were {str(count_cheater)} of {str(len(cheaters))} flagged users kicked"
            f"\n - - - - - - - - - - - - - - - - - - - - - - - - - - - - "
        )
    await ctx.send(text)
...
async def crown_faulty():
    ...
    for team in json_data["teams"]:
        ...
        try:
            loop = asyncio.get_event_loop()
            cheaters = await loop.run_in_executor(
                ThreadPoolExecutor(), function.analyse_team, teamname, ignore_user
            )
        except lichesspy.api.ApiHttpError:
            id_ref[handle][3] = 4
            return False
